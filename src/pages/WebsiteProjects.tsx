...
export default function WebsiteProjects() {
  ...
  const categories = ["All", "React", "Next.js", "E-commerce"];

  const filteredProjects = projects.filter(p => p.category === "Website");
    ...
    <div className="pt-32 pb-24 bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-20">
          ...
          >
            Website <span className="text-accent-magenta">Projects</span>
          </motion.h1>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.2 }}
            className="text-slate-600 text-lg max-w-3xl mx-auto mb-12"
          >
            ...
            {categories.map((cat) => (
              <button
                ...
                className={`px-6 py-2 rounded-full text-sm font-bold transition-all ${
                  filter === cat 
                    ? "bg-accent-magenta text-slate-900 shadow-lg shadow-accent-magenta/20" 
          ...
          {filteredProjects.map((project, index) => (
            ...
            >
              <Link to={project.id === "under-development" ? "#" : `/projects/${project.id}`}>
                <div className="relative aspect-[16/10] rounded-[40px] overflow-hidden mb-8 glass border-slate-100 p-2">
                  <div className="w-full h-full rounded-[32px] overflow-hidden relative">
                    ...
                    {project.id === "under-development" ? (
                      <div className="absolute inset-0 bg-background/80 flex items-center justify-center">
                        <div className="px-8 py-4 glass border-white/20 text-slate-900 rounded-full font-bold flex items-center gap-2">
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {project.tech.map((t) => (
                  <span key={t} className="px-4 py-1.5 rounded-full bg-slate-50 border border-slate-200 text-[11px] font-mono text-slate-600 uppercase tracking-wider">
                    {t}
                  </span>
                ))}
              </div>
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-slate-600 text-lg leading-relaxed max-w-lg mb-6">
                    {project.description}
                  </p>
                  <div className="flex gap-4">
                    <Link 
                      to={`/projects/${project.id}`}
                      className="px-6 py-2 rounded-full glass border-slate-200 text-xs font-bold uppercase tracking-widest hover:bg-slate-50 transition-all"
                    >
                      View Case Study
                    </Link>
                    {project.link !== "#" && (
                ...
                <div className="flex gap-3">
                  {project.github !== "#" && (
                    <a
                      href={project.github}
                      className="w-12 h-12 rounded-full glass border-slate-200 flex items-center justify-center text-slate-900 hover:bg-slate-100 transition-all"
                    >
                      <Github size={20} />
                    </a>
                  )}
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="mt-32 p-12 rounded-[40px] glass border-slate-200 text-center max-w-4xl mx-auto"
        >
          <h3 className="text-2xl font-display font-bold mb-6 text-slate-900">The Beulex Promise</h3>
